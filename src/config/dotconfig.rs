...
fn init_shell_config_folder() -> Result<(), DoraErrors> {
    ...
    if !expected_config_path.exists() {
        // init
        fs::create_dir(expected_config_path).map_err(|e| DoraErrors::IOError(e.to_string()))?;

        Ok(())
    } else {
        if !(expected_config_path.metadata().unwrap().is_dir()) {
            return Err(DoraErrors::IOError(
                "~/.dora folder does not exist".to_string(),
            ));
        }
        Ok(())
    }
...
pub fn read_config_file() -> Result<Config, DoraErrors> {
    ...
    if file_path.exists() {
        let mut file = File::open(file_path).map_err(|e| DoraErrors::IOError(e.to_string()))?;
        let mut contents = String::new();
        file.read_to_string(&mut contents)
            .map_err(|e| DoraErrors::IOError(e.to_string()))?;

        let config: Config =
            toml::from_str(&contents).map_err(|e| DoraErrors::IOError(e.to_string()))?;
        return Ok(config);
    }
    return Err(DoraErrors::IOError(
        "~/.dora/config.toml not found".to_string(),
    ));
}
